...
def load_eva_data(logger, args):
    ...
    if "V1" in file_dir:
        ...
        img_ocr_path = osp.join(args.data_path, f"pkls/dbp_{args.data_split}_ocr.pkl")
    elif "V2" in file_dir:
        split = "dense"
        ...
        img_ocr_path = osp.join(args.data_path, f"pkls/updated_{args.data_choice}_ocr.pkl")
    else:
        # fr_en
    ...
    img_features, imgornot = load_img(logger, ENT_NUM, img_vec_path)
    logger.info(f"image feature shape:{img_features.shape}")
    if args.w_ocr:
        ocr_features, ocrornot = load_ocr(logger, ENT_NUM, img_ocr_path)
        logger.info(f"ocr feature shape:{ocr_features.shape}")
    ...
    if os.path.exists(file_dir + '/modaltype_features.pkl'):
        ...
        modaltype_features = pickle.load(f)
        f.close()
    else:
        enttrinum = []
        for i in range(len(ent2id_dict)):
            enttrinum.append([0, 0])  # triplesnum,relationnum,image,ocr
        for i in range(len(triples)):
            enttrinum[triples[i][0]][0] = enttrinum[triples[i][0]][0] + 1
        for i in range(len(ent2id_dict)):
            rlist = []
            for j in range(len(triples)):
                if triples[j][0] == i:
                    if triples[j][1] not in rlist:
                        rlist.append(triples[j][1])
            enttrinum[i][1] = int(len(rlist))

        modaltype_features = np.array(enttrinum)
        modaltype_features = F.normalize(
            torch.Tensor(modaltype_features)).numpy()
        
        enttrinum1 = []
        if ocrornot is not None:
            for i in range(len(ent2id_dict)):
                enttrinum1.append([0, 0]) 
            for i in range(len(imgornot)):
                # print(i)
                enttrinum1[i][0] = imgornot[i]
            for i in range(len(ocrornot)):
                # print(i)
                enttrinum1[i][1] = ocrornot[i]
        else:
            for i in range(len(ent2id_dict)):
                enttrinum1.append([0]) 
            for i in range(len(imgornot)):
                # print(i)
                enttrinum1[i][0] = imgornot[i]
        modaltype_features1 = np.array(enttrinum1)
        modaltype_features1 = F.normalize(
            torch.Tensor(modaltype_features1)).numpy()    
        
        modaltype_features = np.concatenate((modaltype_features, modaltype_features1), axis=1)
        modaltypefilepath = file_dir + '/modaltype_features.pkl'
        f = open(modaltypefilepath, 'wb')
        pickle.dump(modaltype_features, f)
        f.close()
    

    
    logger.info(f"modaltype feature shape:{modaltype_features.shape}")
    ...
    # att_features = np.concatenate((modaltype_features, att_features), axis=1)
    logger.info(f"attribute concatenate feature shape:{att_features.shape}")
...
def load_img(logger, e_num, path):
    ...
    for i in range(e_num):
        if i in img_dict:
            imgornot.append(int(1))
        else:
            imgornot.append(int(0))
    return img_embd, imgornot


def load_ocr(logger, e_num, path):
    img_dict = pickle.load(open(path, "rb"))
    # init unknown img vector with mean and std deviation of the known's
    imgs_np = np.array(list(img_dict.values()))
    mean = np.mean(imgs_np, axis=0)
    ...
    img_embd = np.array(
        [img_dict[i] if i in img_dict else np.random.normal(mean, std, mean.shape[0]) for i in range(e_num)])
    logger.info(f"{(100 * len(img_dict) / e_num):.2f}% entities have ocr")

    ocrornot = []
    for i in range(e_num):
        if i in img_dict:
            ocrornot.append(int(1))
        else:
            ocrornot.append(int(0))
    return img_embd, ocrornot
