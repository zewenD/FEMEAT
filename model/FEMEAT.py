...
from .FEMEAT_tools import MultiModalEncoder
from .FEMEAT_loss import CustomMultiLossLayer, icl_loss
...
class FEMEAT(nn.Module):
    def __init__(self, kgs, args):
        ...
        if kgs["ocr_list"] is not None:
            self.ocr_features = torch.FloatTensor(kgs["ocr_list"]).cuda()
            ocr_dim = self._get_ocr_dim(kgs)
        else:
        ...
        self.multi_loss_layer = CustomMultiLossLayer(loss_num=7)  # 6
    ...
    def forward(self, batch):
        gph_emb, img_emb, ocr_emb, rel_emb, att_emb, name_emb, char_emb, joint_emb, hidden_states = self.joint_emb_generat(
            only_joint=False)
        gph_emb_hid, rel_emb_hid, att_emb_hid, img_emb_hid, ocr_emb_hid, name_emb_hid, char_emb_hid, joint_emb_hid = self.generate_hidden_emb(
            hidden_states)
        if self.args.replay:
        ...
        in_loss = self.inner_view_loss(gph_emb, rel_emb, att_emb, img_emb, ocr_emb, name_emb, char_emb, batch)
        out_loss = self.inner_view_loss(gph_emb_hid, rel_emb_hid, att_emb_hid, img_emb_hid, ocr_emb_hid, name_emb_hid, char_emb_hid,
                                        batch)
    ...
    def generate_hidden_emb(self, hidden):
        ...
        if hidden.shape[1] >= 6:
            ...
            ocr_emb = None
            if self.args.w_ocr:
                ocr_emb = F.normalize(hidden[:, 4, :].squeeze(1))
                joint_emb = torch.cat([gph_emb, rel_emb, att_emb, img_emb, ocr_emb], dim=1)
            else:
                joint_emb = torch.cat([gph_emb, rel_emb, att_emb, img_emb], dim=1)


        return gph_emb, rel_emb, att_emb, img_emb, ocr_emb, name_emb, char_emb, joint_emb

    def inner_view_loss(self, gph_emb, rel_emb, att_emb, img_emb, ocr_emb, name_emb, char_emb, train_ill):
        # pdb.set_trace()
        ...
        loss_ocr = self.criterion_cl(ocr_emb, train_ill) if ocr_emb is not None else 0
        ...
        total_loss = self.multi_loss_layer([loss_GCN, loss_rel, loss_att, loss_img, loss_ocr, loss_name, loss_char])
        return total_loss

    # --------- necessary ---------------

    def joint_emb_generat(self, only_joint=True):
        gph_emb, img_emb, ocr_emb, rel_emb, att_emb, \
        ...
        if only_joint:
            ...
            return gph_emb, img_emb, ocr_emb, rel_emb, att_emb, name_emb, char_emb, joint_emb, hidden_states
    ...
    def _get_ocr_dim(self, kgs):
        if isinstance(kgs["ocr_list"], list):
            ocr_dim = kgs["ocr_list"][0].shape[1]
        elif isinstance(kgs["ocr_list"], np.ndarray) or torch.is_tensor(kgs["images_list"]):
            ocr_dim = kgs["ocr_list"].shape[1]
        return ocr_dim
