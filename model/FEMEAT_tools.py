...
class MformerFusion(nn.Module):
    def __init__(self, args, modal_num, with_weight=1):
        ...
        self.type_id = torch.tensor([0, 1, 2, 3, 4, 5, 6]).cuda()
...
class MultiModalEncoder(nn.Module):
    ...
    def __init__(self, args,
        ...
        self.ocr_fc = nn.Linear(ocr_feature_dim, img_dim)
    ...
    def forward(self,
        ...
        if self.args.w_img:
            ...
            img_emb = None
        if self.args.w_ocr:
            ocr_emb = self.ocr_fc(ocr_features)
        else:
            ocr_emb = None
        if self.args.w_rel:
        ...
        joint_emb, hidden_states, weight_norm = self.fusion([img_emb, ocr_emb, att_emb, rel_emb, gph_emb, name_emb, char_emb])

        return gph_emb, img_emb, ocr_emb, rel_emb, att_emb, name_emb, char_emb, joint_emb, hidden_states, weight_norm
